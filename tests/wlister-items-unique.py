...
class ParametersUniqueTest(unittest.TestCase):

    def test_parameters_ok_same_order(self):
        r = requests.get('http://localhost/parameters_unique/?var1=val1&var2=val2')
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.content, "OK")

    def test_parameters_ok_different_order(self):
        r = requests.get('http://localhost/parameters_unique/?var2=val2&var1=val1')
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.content, "OK")

    def test_parameters_ko_duplicate01(self):
        r = requests.get('http://localhost/parameters_unique/?var1=ValueNotExpected&var2=val2&var1=val76543')
        self.assertEqual(r.status_code, 404)

    def test_parameters_ko_duplicate02(self):
        r = requests.get('http://localhost/parameters_unique/?var2=val2&var1=val1&var1=val3')
        self.assertEqual(r.status_code, 404)

    def test_parameters_ko_less_parameter(self):
        r = requests.get('http://localhost/parameters_unique/?var1=val1')
        self.assertEqual(r.status_code, 404)
...
class HeadersUniqueTest(unittest.TestCase):
    ...
    def test_headers_ok(self):
        h = {'header-test': 'test', 'header-ok': 'header-ok'}
